...
from src.causal_forest import train_causal_forest, get_feature_importances
...
def main():
    ...
    cf_model = train_causal_forest(X_train_s, T_train, Y_train, n_estimators=100)
    
    print("Getting CATE estimates and confidence intervals...")
    if hasattr(cf_model, 'effect_interval'):
        cate_est = cf_model.effect(X_test_s).flatten()
        lower, upper = cf_model.effect_interval(X_test_s)
        lower = lower.flatten()
        upper = upper.flatten()
        ci_widths = upper - lower
        ci_contains_zero = (lower <= 0) & (upper >= 0)
    else:
        # Fallback if the implementation does not expose effect_interval
        cate_est = cf_model.effect(X_test_s).flatten()
        ci_widths = np.zeros_like(cate_est)
        ci_contains_zero = np.zeros_like(cate_est)
    
    print("Getting feature importances...")
    importances = get_feature_importances(cf_model, feature_names=[f"f{i}" for i in range(12)])
